...
答えを主張するのではなく論証しなければならない問題に関心があります。探索と最適化、データとプライバシー、形式論理、認可の設計、そしてワークフローを正確に保つための小さなツールです。公開しているものはどれも同じ作り方をしています。主張はREADMEに書き、根拠はリポジトリに置き、テストはCIで動かします。
